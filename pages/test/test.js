...
Page({
  ...
  data: {
    ...
    ctx: '',
    ctx_shadow:'',//隐藏canvas 上下文
    canvasWidth: 0,
    canvasHeight: 0,
    shadow_canvasWidth:0,
    shadow_canvasHeight:0,
    transparent: 1, // 透明度
    selectColor: 'black',
    lineColor: '#1A1A1A', // 颜色
    lineSize: 1.5,  // 笔记倍数
  ...
  onLoad: function (options) {
    ...
    let ctx_shadow = wx.createCanvasContext('shadowCanvas')
    console.log(ctx_shadow)
    this.setData({
      ctx: ctx,
      ctx_shadow:ctx_shadow
    })
    ...
    query.select('.shadow-canvas').boundingClientRect(rect => {
      console.log(rect)
      // this.setShadowCanvasBg('red')
      this.setData({
        shadow_canvasWidth: rect.width,
        shadow_canvasHeight: rect.height
      })
    }).exec();
  ...
  retDraw() {
    ...
    this.data.ctx.draw()
    this.data.ctx_shadow.clearRect(0, 0, 700, 730)
    this.data.ctx_shadow.draw()
    //设置canvas背景
    // this.setCanvasBg("#fff");
  },
  ...
  setShadowCanvasBg(color) {
    console.log(999);
    /* 将canvas背景设置为 白底，不设置  导出的canvas的背景为透明 */
    //rect() 参数说明  矩形路径左上角的横坐标，左上角的纵坐标, 矩形路径的宽度, 矩形路径的高度
    //这里是 canvasHeight - 4 是因为下边盖住边框了，所以手动减了写
    this.data.ctx_shadow.rect(0, 0, this.data.canvasWidth, this.data.canvasHeight - 4);
    // ctx.setFillStyle('red')
    this.data.ctx_shadow.setFillStyle(color)
    this.data.ctx_shadow.fill()  //设置填充
    this.data.ctx_shadow.draw()    //开画

  },
  //点击保存按钮，保存到相册
  saveCanvasAsImg() {
    //拿到图片数据后，生成另一个隐藏的canvas，该canvas是竖屏状态，然后绘制图片，最后保存图片到本地
    let ctx_shadow = this.data.ctx_shadow
    let ctx = this.data.ctx
    let canvasWidth = this.data.canvasWidth
    let canvasHeight = this.data.canvasHeight
    let _this = this
    //旋转canvas
    // ctx_shadow.translate(canvasWidth / 2, canvasHeight / 2)
    // ctx_shadow.rotate(90 * Math.PI / 180)
    ...
    wx.canvasToTempFilePath({
      ...
      success(res) {
        ...
        //   }
        // })
        //2、将图片画在隐藏的canvas，旋转调整后再保存图片到相册
        wx.getImageInfo({
          src: res.tempFilePath,
          success (res) {
            console.log(res)
            let rate = _this.data.canvasWidth / _this.data.canvasHeight
            ctx_shadow.translate( 50, 50)
            ctx_shadow.rotate(-90 * Math.PI / 180)
            ctx_shadow.drawImage(res.path, 0, 0, 440, 400)
            ctx_shadow.draw()
  ...
  saveShadow(){//将隐藏的canvas保存为图片
    wx.canvasToTempFilePath({
      canvasId: 'shadowCanvas',
      fileType: 'png',
      quality: 1, //图片质量
      success(res) {
        console.log(res.tempFilePath, 'canvas生成图片地址');
